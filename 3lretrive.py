...
def read_linear(start):
	f=open("data/3level.txt","r")
	print(start)
	f.seek(int(start.split()[0]))
	for line in f:
		row=get_key(line)
		print(row,keyword)
		if (row==keyword):
			...
			print(keyword, "found")
			break
	print("failed_badly if not found")
...
if __name__=="__main__":
	...
	middle=None
	prev_jump=[0]
	f=open("data/3ltest.txt","r")
	b_file=open("data/3level.txt","r")
	for l in f:
